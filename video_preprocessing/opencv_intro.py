...
def process_frame(frame, apply_filters=True, crop=None, resize=None):
    """
    Process a single video frame with optional cropping, resizing, and filters.
    
    Args:
        frame: Input video frame.
        apply_filters: Apply effects like edge detection (default: True).
...
def main():
    parser = argparse.ArgumentParser(description='Process video with optional CSV frame filtering and playback speed adjustment')
    ...
    parser.add_argument('--speed', type=float, default=1.0, help='Speed multiplier for playback (e.g., 2.0 for 2x speed)')
    ...
    output_fps = max(1, int(original_fps * args.speed))  # Ensure FPS is at least 1
    ...
    while True:
        ...
        if not frame_filter or frame_filter.get(frame_number, 0) == 1:
            # Process the frame
            processed_frame = process_frame(frame, args.filters, args.crop, args.resize)
